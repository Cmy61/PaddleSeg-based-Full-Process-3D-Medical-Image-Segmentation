...
<div class="myTable-wrap">
    ...
    <div class="myTable-wrap-body">
        <el-table
            ...
            <el-table-column
                ...
                :label="labelData[i]"
                :width="widthData[i]"
                v-if="(item=='set_id' || item=='set_name' || item=='build_time' || item=='remark') && tableData.length!==0"
            >
            </el-table-column>
    ...
    <el-dialog title="信息" :visible.sync="dialogTableVisible" :before-close="handleDialogClose"> 
        ...
        <el-descriptions border v-if="dialog_mode==1" :column="2">
            <el-descriptions-item v-for="(val,key,idx) in currentRow" :key="labelData[idx]+''+idx" :label="labelData[idx]" v-if="key=='set_id' || key=='set_name' || key=='remark' || key=='build_time'">
                {{val}}
        ...
        <el-form ref="form" :model="currentRow_temp" label-width="100px" v-if="dialog_mode==2">
                ...
                <el-form-item :label="labelData[idx]" v-for="(val,key,idx) in currentRow_temp" :key="labelData[idx]" v-if="key=='set_id' || key=='set_name' || key=='remark'">
        ...
        <el-form ref="form" :model="currentRow_temp" label-width="100px" v-if="dialog_mode==3">
                ...
                <el-form-item :label="labelData[idx]" v-for="(key,idx) in propsData" :key="labelData[idx]" v-if="key=='set_id' || key=='set_name' || key=='remark'" :required="key=='set_id' || key=='set_name'">
                <!-- $set(currentRow_temp, key, $event.target.value); -->
                    <el-col :span="15">
