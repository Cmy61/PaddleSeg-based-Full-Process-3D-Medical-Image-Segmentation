...
<div class="myTable-wrap">
    <div class="myTable-wrap-header">
        <el-input placeholder="请输入内容" v-model="search_text" class="input-with-select">
            <el-select v-model="search_select" slot="prepend" placeholder="请选择" style="width:130px;">
        ...
        <template v-if="right_level==1 || right_level==0">
            <el-button @click="refresh" icon="el-icon-refresh-left" circle style="margin-left:44%;"></el-button>
        ...
        <el-table
            ...
            >
                <template slot-scope="scope">
                    <span v-if="scope.row[propsData[i]] instanceof Array">
                        {{ scope.row[propsData[i]].join(',') }}
                    </span>
                    <span v-else>
            ...
            <el-table-column
                ...
                width="270">
                <template slot-scope="scope">
                    <el-button @click="enter(scope.row,(currentPage-1)*pageSize + scope.$index)" type="text" size="small" icon="el-icon-view">进入</el-button>
    ...
    <el-dialog title="信息" :visible.sync="dialogTableVisible" :before-close="handleDialogClose"> 
        ...
        <el-form ref="form" :model="currentRow_temp" label-width="100px" v-if="dialog_mode==2">
                ...
                <el-form-item :label="labelData[idx]" v-for="(val,key,idx) in currentRow_temp" :key="labelData[idx]" v-if="key=='project_name' || key=='remark'">
        ...
        <el-form ref="form" :model="currentRow_temp" label-width="100px" v-if="dialog_mode==3">
                ...
                <el-form-item :label="labelData[idx]" v-for="(val,key,idx) in currentRow_temp" :key="labelData[idx]" v-if="key=='project_id' || key=='project_name' || key=='remark'" :required="key=='project_id' || key=='project_name'">
                <!-- $set(currentRow_temp, key, $event.target.value); -->
                    <el-col :span="15">
...
methods: {
    ...
    enter(row,idx){
        ...
        this.$router.push({ 
            ...
            params: {
                ...
                project_build_time:row.project_build_time,
                sets_id:row.sets_id,
                remark:row.remark
            }
        })
    ...
    async comfirm_add(){
        for (let item in this.currentRow_temp){
            ...
            if((item=='project_id' || item=='project_name') && this.currentRow_temp[item]==null){
