    ...
    <div class="myTable-wrap">
        <div class="myTable-wrap-header">
            ...
            <template v-if="right_level == 2">
                <el-button @click="refresh" icon="el-icon-refresh-left" circle style="margin-left:34%;"></el-button>
                <!-- <el-button @click="add" icon="el-icon-plus" style="margin-left:2%;">添加</el-button> -->
            </template>
        </div>
        <div class="myTable-wrap-body">
            <el-table stripe v-loading="loading" ref="singleTable"
                :data="tableData.slice((currentPage - 1) * pageSize, currentPage * pageSize)" border style="width: 100%;"
                highlight-current-row @current-change="handleCurrentRowChange" :key="table_key" height="91%"
                :header-cell-style="{ background: '#f4f3f9', color: '#606266' }">
                <template slot="empty">
                    <el-empty description="暂无数据"></el-empty>
                </template>

                <el-table-column v-for="(item, i) in propsData" :key="i" :prop="propsData[i]" :label="labelData[i]"
                    :width="widthData[i]">
                    <template slot-scope="scope">
                        
                        <span v-if="i == 0">
                            {{ scope.row[propsData[i]] }}
                        </span>
                        <span v-else-if="i == 5 || i == 8 || i == 9">
                            无含义
                        </span>
                        <el-tag v-else-if="(i== 7 || i == 11 || i == 12 || i == 13 || i == 15) && (scope.row[propsData[i]])">
                            {{ scope.row[propsData[i]] }}
                        </el-tag>
                        <el-tag type="info" v-else-if="!scope.row[propsData[i]]">
                            暂无
                        </el-tag>
                        <el-tag type="warning" v-else-if="scope.row[propsData[i]] <= organ_normal_volume[i][0]">
                            {{ scope.row[propsData[i]] }}
                        </el-tag>
                        <el-tag type="success" v-else-if="scope.row[propsData[i]] <= organ_normal_volume[i][1]">
                            {{ scope.row[propsData[i]] }}
                        </el-tag>
                        <el-tag type="danger" v-else>{{ scope.row[propsData[i]] }}</el-tag>
                    </template>
            ...
            <el-pagination @size-change="handleSizeChange" @current-change="handleCurrentChange" :current-page="currentPage"
                :page-sizes="[4, 5, 6, 7]" :page-size="pageSize" layout="total, sizes, prev, pager, next, jumper"
                :total="tableData.length">
            </el-pagination>
        </div>
        <div style="width: 93%;margin-top: 1%;margin-left:3.2%;font-size: 8px;">
				以下为上面正常体积大小判定区间，单位为立方厘米。
				<span style="font-weight:bold;">脾脏</span> 50 ~ 250；
				<span style="font-weight:bold;">右肾</span> 100 ~ 200；
				<span style="font-weight:bold;">左肾</span> 100 ~ 150；
				<span style="font-weight:bold;">胆囊</span> 30 ~ 50；
				<span style="font-weight:bold;">肝脏</span> 1200 ~ 1600；
				<span style="font-weight:bold;">胰腺</span> 60 ~ 100；
				<span style="font-weight:bold;">膀胱</span> 300 ~ 500；
			</div>
    </div>
        ...
        setCurrentRow(row) {
            if (row == null) {
                ...
                for (let i = 0; i < this.propsData.length; ++i) {
                    this.currentRowIndex[this.propsData[i]] = null;
                }
            }
